...
> **ネイティブ部品の追従（v0.4.2〜）**: RicUI のテーマは基本的に CSS 変数による
> 塗り分けですが、スクロールバー・`<select>` のドロップダウン・checkbox/radio の
> チェックマーク・`<input type="date">` の日付ピッカーなど、ブラウザ（OS）が直接
> 描画する部分は CSS 変数では追従しません。これらは `color-scheme`
> （`light`/`dark`）で制御され、`make_css_vars` の出力に自動で含まれるため、
> `create_ui_page` で組んだページなら特別な対応なしにテーマへ追従します。
> `create_ui_page` を通さず `css_for` の 3 点セットで一部だけ埋め込む場合も、
> `make_css_vars` を当てた要素とその配下が対象になります（`:root` には乗らない
> ので、テーマの異なる島同士が干渉することはありません）。
